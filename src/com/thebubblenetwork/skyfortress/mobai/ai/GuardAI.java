...
public class GuardAI extends CreatureAI<PigZombie> {
    ...
    public void onTarget(EntityTargetEvent e) {
        if (!(e.getTarget() instanceof Player) || SkyFortress.getInstance().getGame().isSpectating((Player) e.getTarget()) || e.getTarget().getLocation().distance(getCreature().getLocation()) > 15) {
            e.setCancelled(true);
        }
    }
    ...
    public void onDeath(EntityDeathEvent e) {
        ...
        for (DeathListener listener : deathListenerSet) {
            listener.onDeath();
        }
    }
